...
const WorkspaceHeader: React.FC<WorkspaceHeaderProps> = ({
  ...
  const handleCloseElmErrorModal = (): void => {
    setShowElmErrorModal(false);
    setElmErrors([]);
  };

  const handleDownload = async (version: string): Promise<void> => {
    setMenuAnchorEl(null);
  ...
  const handleViewCQL = (version: string): void => {
    setMenuAnchorEl(null);
    setShowCQLModal(true);
    setDataModel({ name: 'FHIR', version });
  };

  const handleDataModelMenuSelected = async (version: string): Promise<void> => {
    const menuId = menuAnchorEl?.id;
    switch (menuId) {
      case 'view-cql':
  ...
  return (
    <>
      ...
      {showCQLModal && (
        <CQLModal
          handleCloseModal={() => setShowCQLModal(false)}
          artifact={artifact as never}
          dataModel={dataModel as never}
        />
      )}
    </>
  );
