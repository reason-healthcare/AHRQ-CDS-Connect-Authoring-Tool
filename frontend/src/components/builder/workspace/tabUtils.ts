...
interface TabMetadataMap {
  expTreeInclude: TabMetadata;
  expTreeExclude: TabMetadata;
  subpopulations: TabMetadata;
  baseElements: TabMetadata;
  recommendations: TabMetadata;
...
export const getTabMetadata = (artifact: Artifact, externalCqlListLength: number): TabMetadataMap => {
  const { expTreeInclude, expTreeExclude, subpopulations, baseElements, recommendations, parameters, errorStatement } =
    artifact;
  const metadata: TabMetadataMap = {} as TabMetadataMap;
  const allElements = getAllElements(artifact) ?? [];
  const names = getElementNames(allElements);
  const namedParameters = (parameters || []).filter(({ name }) => name && name.length > 0);
  ...
  metadata.subpopulations = {
    hasContent:
      ...
      ((subpopulations || []).length === 3 &&
        ((subpopulations || [])[2]?.subpopulationName !== 'Subpopulation 1' ||
          ((subpopulations || [])[2]?.childInstances || []).length > 0)),
  ...
  metadata.handleErrors = {
    hasContent: (errorStatement?.ifThenClauses?.[0]?.ifCondition?.value ?? null) != null,
    hasError: errorStatementHasWarnings(errorStatement || { ifThenClauses: [] }, expTreeInclude, expTreeExclude)
  };
