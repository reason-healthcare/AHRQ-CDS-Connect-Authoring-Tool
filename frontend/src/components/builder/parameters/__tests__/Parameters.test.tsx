...
const mockArtifactWithParameterUsed = {
  ...
  expTreeInclude: {
    ...((mockArtifact as { expTreeInclude?: Record<string, unknown> }).expTreeInclude || {}),
    childInstances: [
      {
...
interface RenderComponentProps {
  artifact?: unknown;
  handleUpdateParameters?: jest.Mock;
  [key: string]: unknown;
}

describe('<Parameters />', () => {
  const renderComponent = ({ artifact = mockArtifactWithParameters, ...props }: RenderComponentProps = {}) =>
    render(
  ...
  it('can update a parameter', async () => {
    ...
    expect(handleUpdateParameters).toHaveBeenCalledWith(newParameters);
  });
  ...
  it('can delete a parameter', async () => {
    ...
    expect(handleUpdateParameters).toHaveBeenCalledWith(newParameters.slice(1));
  });
