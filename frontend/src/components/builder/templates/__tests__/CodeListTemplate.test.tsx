...
import CodeListTemplate, { CodeListTemplateProps, Code } from '../CodeListTemplate';
...
describe('<CodeListTemplate />', () => {
  const renderComponent = (props: CodeListTemplateProps | {} = {}) =>
    render(
      <CodeListTemplate
        codes={('codes' in props ? props.codes : []) as Code[]}
        handleDeleteCode={jest.fn()}
        {...Object.fromEntries(Object.entries(props).filter(([key]) => key !== 'codes'))}
      />
    );
  ...
  it('calls handleDeleteCode on code delete', async () => {
    ...
    expect(handleDeleteCode).toHaveBeenCalledWith(code1);
  });
});
