...
interface RenderComponentProps {
  ...
  deleteLists?: jest.Mock;
  editInstance?: jest.Mock;
  listInstance?: Instance;
  updateInstanceModifiers?: jest.Mock;
  updateLists?: jest.Mock;
  [key: string]: unknown;
}

describe('<ListGroup />', () => {
  const genericBaseElementListTemplateInstance = templateInstanceToInstance(
    createTemplateInstance(genericBaseElementListInstance)
  );
  const genericBaseElementUseTemplateInstance = templateInstanceToInstance(
    createTemplateInstance(genericBaseElementUseInstance)
  );

  const renderComponent = ({
    baseElements = [],
    expTreeInclude = mockArtifact.expTreeInclude,
    ...props
  ...
  it('can be deleted when not in use', async () => {
    ...
    expect(deleteLists).toHaveBeenCalled();
  });
  ...
  it('should not have indent button on children when the list is an intersect/union type', async () => {
    ...
    unionElement.childInstances = [templateInstanceToInstance(createTemplateInstance(genericInstance))];
    const { container } = renderComponent({
  ...
  it('should have indent button on children when the list is an and/or type', async () => {
    const andElement = templateInstanceToInstance(createTemplateInstance(genericBaseElementListAndInstance));
    andElement.childInstances = [templateInstanceToInstance(createTemplateInstance(genericInstance))];
    const { container } = renderComponent({
  ...
  it('should call updateLists when list name is updated', async () => {
    ...
    expect(updateLists).toHaveBeenCalled();
  });
  ...
  it('should call updateLists when list comment is updated', async () => {
    ...
    await waitFor(() => expect(updateLists).toHaveBeenCalled());
  });
  ...
  it('should render return type without a check for and/or lists with invalid return type', () => {
    const listInstance = templateInstanceToInstance(createTemplateInstance(genericBaseElementListAndInstance));
  ...
  it('should give And/Or conjunction options for and/or lists', async () => {
    const listInstance = templateInstanceToInstance(createTemplateInstance(genericBaseElementListAndInstance));
    listInstance.childInstances = [templateInstanceToInstance(genericInstance)]; // a child needs to be present for conjunction types to render
    const { container } = renderComponent({ listInstance });
    await waitFor(() => {
      const dropdown = within(container).getAllByRole('combobox', { name: '' })[0];
      return userEvent.click(dropdown);
  ...
  describe('alerts and warnings', () => {
    ...
    it('should have a warning on base element list when in use and the use is modified', () => {
      ...
      if (nameField) (nameField as { id?: string; value?: unknown }).value = 'UnionListName';
    ...
    it('should have a duplicate name warning when a different element (non use) has the same name', () => {
      const instanceWithSameName = templateInstanceToInstance(createTemplateInstance(genericInstance));
      const nameField = getFieldWithId(instanceWithSameName.fields, 'element_name');

      instanceWithSameName.uniqueId = 'id-for-instance-1';
      if (nameField) (nameField as { id?: string; value?: unknown }).value = 'UnionListName'; // Same name as Base Element List
    ...
    it('should have an intersection warning when different types are intersected', () => {
      const observationList = templateInstanceToInstance(createTemplateInstance(genericInstance));
      const procedureList = templateInstanceToInstance(createTemplateInstance(genericInstance));
      ...
      if (intersectionListInstance.fields && intersectionListInstance.fields[0]) {
    ...
    it('should have a boolean return type warning when different types are combined in an and/or list', async () => {
      ...
      if (observationList2.fields && observationList2.fields[0]) {
      ...
      if (orListInstance.fields && orListInstance.fields[0]) {
    ...
    it('should render a simple "Has Errors" warning when collapsed if child has errors', async () => {
      const listInstance = templateInstanceToInstance(createTemplateInstance(genericBaseElementListAndInstance));
      const nonBooleanChild = _.cloneDeep(templateInstanceToInstance(createTemplateInstance(genericInstance)));
