...
interface RenderComponentProps {
  ...
  modifiersByInputType?: Record<string, unknown[]>;
...
describe('<SelectModifierAction />', () => {
  ...
  it('renders add modifiers button when there are relevant modifiers', () => {
    const elementInstance: Instance = { returnType: 'list_of_conditions', modifiers: [] }; // cannotHaveModifiers is not specified, so they are allowed
  ...
  it('renders modifier select modal when clicking add modifiers', async () => {
    const elementInstance: Instance = { returnType: 'list_of_conditions', fields: [], modifiers: [] }; // cannotHaveModifiers is not specified, so they are allowed
    const modifiersByInputType = { list_of_conditions: [{ name: 'TestModifier' }] };
