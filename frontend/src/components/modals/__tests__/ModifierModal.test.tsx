...
import type { ModifierTree } from '../ModifierModal/types';
...
interface RenderComponentProps {
  ...
  elementInstance?: unknown;
  handleCloseModal?: jest.Mock;
  handleUpdateModifiers?: jest.Mock;
  hasLimitedModifiers?: boolean;
  modifierToEdit?: unknown;
  [key: string]: unknown;
}

describe('<ModifierModal />', () => {
  const apiKey = 'api-1234';
  const renderComponent = ({ artifact = mockArtifact, useObservation = false, ...props }: RenderComponentProps = {}) =>
    render(
      <Provider store={createStore((x: RootState) => x, { artifacts: { artifact }, vsac: { apiKey } } as RootState)}>
        <ModifierModal
          elementInstance={templateInstanceToInstance(
          ...
          modifierToEdit={null}
          {...(props.modifierToEdit ? { modifierToEdit: props.modifierToEdit as ModifierTree } : {})}
          {...Object.fromEntries(Object.entries(props).filter(([key]) => key !== 'modifierToEdit'))}
        />
      </Provider>
    );
  ...
  beforeEach(() => {
    nock('http://localhost')
      ...
      .reply(200, (uri: string) => (/Condition/.test(uri) ? [mockConditionResourceR4] : [mockObservationResourceR4]))
    ...
    (updateArtifact as jest.Mock).mockImplementation(() => ({ type: 'foo' }));
  });
  ...
  describe('Select Modifiers', () => {
    ...
    it('can add a single modifier', async () => {
      ...
      const existsModifier = (mockModifiers as Array<{ id?: string; [key: string]: unknown }>).find(
        ({ id }) => id === 'BooleanExists'
      );
    ...
    it('can add more than one modifier', async () => {
      ...
      const notModifier = (mockModifiers as Array<{ id?: string; [key: string]: unknown }>).find(
        ({ id }) => id === 'BooleanNot'
      );
      expect(handleUpdateModifiers).toHaveBeenCalledWith(
        [
  ...
  describe('Build Modifier', () => {
    ...
    it('cannot navigate to the Build Modifier view if the return type is not supported', () => {
      renderComponent({ elementInstance: (instanceTree as { childInstances?: unknown[] }).childInstances?.[0] });
    ...
    it('can edit a custom modifier', async () => {
      ...
      renderComponent({
        artifact: { ...(mockArtifact as Record<string, unknown>), fhirVersion: '4.0.1' },
