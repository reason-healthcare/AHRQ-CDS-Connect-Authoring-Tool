...
describe('<ArtifactTable />', () => {
  const artifactsMock: ArtifactMock[] = [
    {
      _id: 'artifact1',
  ...
  interface RenderComponentProps {
    ...
    handleDeleteArtifact?: jest.Mock;
    handleUpdateArtifact?: jest.Mock;
    handleDuplicateArtifact?: jest.Mock;
    [key: string]: unknown;
  }
  ...
  it('allows deleting of artifacts', async () => {
    ...
    expect(handleDeleteArtifact).toHaveBeenCalledWith(artifactsMock[1]);
  });
  ...
  it('allows duplication of artifacts', async () => {
    ...
    expect(handleDuplicateArtifact).toHaveBeenCalledWith({ _id: artifactsMock[1]._id });
  });
});
