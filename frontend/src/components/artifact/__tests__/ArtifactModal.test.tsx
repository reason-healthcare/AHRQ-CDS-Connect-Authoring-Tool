...
interface ArtifactMock {
  ...
  purpose: string;
  usage: string;
  strengthOfRecommendation: { strengthOfRecommendation: string | null; code: string; system: string };
  qualityOfEvidence: { qualityOfEvidence: string | null; code: string; system: string };
  copyright: string;
  approvalDate: string | null;
  lastReviewDate: string | null;
  effectivePeriod: { start: string | null; end: string | null };
  topic: unknown[];
  author: unknown[];
  reviewer: unknown[];
  endorser: unknown[];
  relatedArtifact: unknown[];
}

const artifactMock: ArtifactMock = {
  _id: 'artifact2',
...
interface RenderComponentProps {
  artifactEditing?: ArtifactMock | null;
  handleAddArtifact?: jest.Mock;
  handleCloseModal?: jest.Mock;
  handleUpdateArtifact?: jest.Mock;
  [key: string]: unknown;
}
...
const waitForInputValueChange = (input: HTMLElement, value: string, expectedValue?: string) => {
...
describe('<ArtifactModal />', () => {
  ...
  describe.skip('cpg fields', () => {
    ...
    it('can fill out the CPG form', async () => {
      ...
      await act(async () => {
        const reviewerInput = document.querySelector('input[name="reviewer[0].reviewer"]') as HTMLInputElement;
        const authorInput = document.querySelector('input[name="author[0].author"]') as HTMLInputElement;
        const endorserInput = document.querySelector('input[name="endorser[0].endorser"]') as HTMLInputElement;
        ...
        await waitForInputValueChange(endorserInput, 'NewArtifactEndorser');
      });
      ...
      await act(async () => {
        ...
        const urlTextarea = document.querySelector('textarea[name="relatedArtifact[0].url"]') as HTMLTextAreaElement;
        const citationTextarea = document.querySelector(
        ...
        await waitForInputValueChange(citationTextarea, 'NewArtifactCitationCitation');
      });
