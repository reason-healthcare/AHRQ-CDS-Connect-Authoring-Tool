...
const backgroundImage = require('./cds-connect-background.jpg') as string;

const buttonStyles = (theme: Theme, color: string) => {
  const common = theme.palette.common;
  return {
    backgroundColor: common[color],
    borderColor: `${common[color]} transparent transparent`,
    '&:hover, &:focus, &:active': {
      backgroundColor: common[color],
      color: common.white
    }
...
export default makeStyles(
  (theme: Theme) => {
    ...
    return {
      ...
      landingHeader: {
        backgroundColor: common.grayDark
      },
      whatsNewHeader: {
        backgroundColor: common.redLight,
      ...
      whatsNewDisplayImage: {
        ...
        '& img': {
          width: '30em',
          border: `1px solid ${common.grayLight}`,
          [theme.breakpoints.down('md')]: {
            width: '100%'
          }
        }
