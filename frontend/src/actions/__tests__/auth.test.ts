...
describe('auth actions', () => {
  ...
  describe('get current user', () => {
    it('sends a GET request to find the current user', () => {
      ...
      return Promise.resolve(store.dispatch(actions.getCurrentUser())).then(() => {
  ...
  describe('logging in', () => {
    it('dispatches a LOGIN_SUCCESS action upon a successful login attempt', () => {
      ...
      return Promise.resolve(store.dispatch(actions.loginUser(username, password))).then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
    });
    ...
    it('dispatches a LOGIN_FAILURE action upon an unsuccessful login attempt', () => {
      ...
      nock('http://localhost')
        ...
        .reply(403, function () {
          ...
          (this.req as any).response.statusMessage = 'Invalid credentials';
          return { status: 403 };
        });
  ...
  describe('logging out', () => {
    it('dispatches a LOGOUT_SUCCESS action upon successful logout', () => {
      ...
      return Promise.resolve(store.dispatch(actions.logoutUser())).then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
    });
    ...
    it('dispatches a LOGOUT_FAILURE action upon an unsuccessful logout', () => {
      ...
      nock('http://localhost')
        ...
        .reply(500, function () {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (this.req as any).response.statusMessage = 'Whoops!';
          return { status: 500 };
        });
  ...
  describe('get user settings', () => {
    it('dispatches a USER_SETTINGS_SUCCESS action upon successful attempt to get settings', () => {
      ...
      return Promise.resolve(store.dispatch(actions.getSettings())).then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
    });
    ...
    it('dispatches a USER_SETTINGS_FAILURE action upon unsuccessful attempt to get settings', () => {
      ...
      nock('http://localhost')
        ...
        .reply(500, function () {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (this.req as any).response.statusMessage = 'Internal error';
          return { status: 500 };
        });
  ...
  describe('update user settings', () => {
    it('dispatches an UPDATE_USER_SETTINGS_SUCCESS action upon successfully updating settings', () => {
      ...
      return Promise.resolve(store.dispatch(actions.updateSettings(settings))).then(() => {
        expect(store.getActions()).toEqual(expectedActions);
      });
    });
