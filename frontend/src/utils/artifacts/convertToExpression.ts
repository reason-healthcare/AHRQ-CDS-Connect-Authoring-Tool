...
function getOperation(operator: string): string {
  switch (operator) {
    case '<':
      return 'less than';
    case '<=':
...
function getExpressionSentenceValue(modifier: Modifier): ExpressionSentenceValue | null {
  // TODO Eventually move the object for each expression onto modifier.js objects. This will likely require a migration.
  const expressionSentenceValues: Record<string, ExpressionSentenceValue> = {
    ...
    switch (modifier.id) {
      ...
      case 'Qualifier': {
        ...
        if (!valueSetText) {
          ...
          if (code) {
            if (code.display) {
              valueSetText = code.display;
            } else {
              valueSetText = `${code.code} (${code.system})`;
            }
          }
        }
      ...
      case 'ConvertObservation': {
        const conversionDescription =
          (modifier.values?.description as string) || (modifier.values?.value as string) || '';
      ...
      case 'AfterTimePrecise': {
        ...
        const time = (modifier.values?.time as string) || '';
        expressionSentenceValues[modifier.id].modifierText += ` ${time.split('T')[1] || ''}`;
        break;
      }
      ...
      case 'AfterDateTimePrecise': {
        ...
        const date = (modifier.values?.date as string) || '';
        expressionSentenceValues[modifier.id].modifierText += ` ${date.split('@')[1] || ''}`;
        if (modifier.values?.time) {
          const time = (modifier.values.time as string) || '';
          expressionSentenceValues[modifier.id].modifierText += ` at ${time.split('T')[1] || ''}`;
        }
        break;
      }
      ...
      case 'BooleanComparison': {
        expressionSentenceValues[modifier.id].modifierText = (modifier.values?.value as string) || '';
        break;
      }
      case 'CheckExistence': {
        expressionSentenceValues[modifier.id].modifierText = (modifier.values?.value as string) || '';
  ...
  if (modifier.type === 'ExternalModifier') {
    const nameIndex = modifier.name.indexOf('(');
    return {
      modifierText: `with ${nameIndex > 0 ? modifier.name.substring(0, nameIndex - 1) : modifier.name}`,
      leadingText: '',
      type: 'post',
      id: modifier.id
    };
...
function updateExpressionsForCountExpression(expressionArray: ExpressionSentenceValue[]): ExpressionSentenceValue[] {
...
function addVSandCodeText(expressionArray: ExpressionLabel[], valueSets: ValueSet[], codes: Code[]): ExpressionLabel[] {
...
function addElementNames(
  expressionArray: ExpressionLabel[],
  elementNames: ElementName[],
  type: string,
  isBaseElementAndOr: boolean
): ExpressionLabel[] {
...
function addlabel(
  expressionArray: ExpressionLabel[],
  expression: ExpressionSentenceValue,
  ...
  if (expression.modifierText) {
    expressionArray.push({ label: expression.modifierText, isTag: true });
  }
  return expressionArray;
}

function getOrderedExpressionSentenceArrayForAgeRange(
  expressionArray: ExpressionSentenceValue[],
  ageFields: Field[]
): ExpressionLabel[] {
  let orderedExpressionArray: ExpressionLabel[] = [];
  ...
  const minAgeField = getFieldWithId(ageFields, 'min_age') as Field | undefined;
...
function getOrderedExpressionSentenceArrayForGender(genderFields: Field[]): ExpressionLabel[] {
  const orderedExpressionArray: ExpressionLabel[] = [];
  orderedExpressionArray.push({ label: "The patient's", isTag: false });
...
function orderExpressionSentenceArray(
  ...
  if ((type === 'And' || type === 'Or') && !isBaseElementAndOr) {
    const andOrExpressionArray: ExpressionLabel[] = [];
  ...
  const checkExistenceExpression = expressionArray.find(expression => {
    ...
    return expression.modifierText && nulls.indexOf(expression.modifierText) !== -1;
  });
...
export default function convertToExpression(
  ...
  referenceElementName: string | null = null
): ExpressionLabel[] {
  const expressionSentenceArray = expressionsArray.reduce(
    (accumulator: ExpressionSentenceValue[], currentExpression) => {
      const expressionSentenceValue = getExpressionSentenceValue(currentExpression);
      if (expressionSentenceValue && !_.isEqual(expressionSentenceValue, { type: '' })) {
