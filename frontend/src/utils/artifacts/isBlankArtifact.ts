 ...
 * @param artifact - the artifact to check for blankness
 * @return true if the artifact has no meaningful data, false otherwise
 */
export default function isBlankArtifact(artifact: Artifact | null | undefined): boolean {
  if (artifact == null) {
    return true;
  }
  ...
  if (artifact.expTreeExclude?.childInstances && artifact.expTreeExclude.childInstances.length > 0) {
    return false;
  }
  // If it has more than one recommendation, it is NOT blank
  if ((artifact.recommendations?.length || 0) > 1) {
    return false;
  }
  ...
  if (artifact.recommendations?.length === 1) {
    ...
    if (
      ...
      r.rationale !== '' ||
      r.comment !== '' ||
      (r.subpopulations?.length || 0) > 1
    ) {
      return false;
    }
  }
  // If it has more than three subpopulations, it is not blank
  if ((artifact.subpopulations?.length || 0) > 3) {
    return false;
  }
  ...
  if (artifact.subpopulations?.length === 3) {
    ...
    if ((subpop.childInstances?.length || 0) > 0) {
      return false;
    }
  }
  // If it has any parameters, it is NOT blank
  if ((artifact.parameters?.length || 0) > 0) {
    return false;
  }
  ...
  if ((artifact.errorStatement?.ifThenClauses?.length || 0) > 1) {
    return false;
  }
  ...
  if (artifact.errorStatement?.ifThenClauses?.length === 1) {
    ...
    if ((firstIfThenClause.statements?.length || 0) > 0 || firstIfThenClause.thenClause !== '') {
      return false;
    }
