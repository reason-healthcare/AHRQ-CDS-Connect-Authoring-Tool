...
const createMockArtifact = (name: string, version: string, lastEdited: string, created: string): MockArtifact => {
  return {
    name: name,
    version: version,
...
const createDateSequence = (numDates: number): string[] => {
  const dates: string[] = [];
...
describe('artifactSort', () => {
  ...
  describe('sortByVersion: Greater APR Precedes Lesser APR', () => {
    ...
    it('should sort artifacts by APR version strings (REVERSE), then default to STRING comparison, then to EMPTY strings', () => {
      const unorderedVersions = orderedVersions.map(v => v).reverse();
